...
function algo_JADE(f::Function,plotf::Function,vp::VarParam,ap::JADEParam,op::OptParam)
    ...
    nftot=countfeasible(pop,eps)

    maxeq=zeros(neq)
    maxineq=zeros(nineq)
    maxviol!(pop,maxeq,maxineq)

    (imin,itruemin)=popleaderpercent(p,pop,maxeq,maxineq,eps)

    copy!(indopt,pop.ind[imin])
    copy!(indtrueopt,pop.ind[itruemin])

    isfeasible(indtrueopt,eps) ? fopt[1]=indtrueopt.f[1] : fopt[1]=1.0E38
...
@notime begin
    while it<maxit
        ...
        nevals += nind
        @notime nfeasible=countfeasible(poptrial,eps)
        nftot += nfeasible
        ...
        oldeps=eps
        (eps,upd)=epsilonupdate(it,maxit,eps,epsmin,epsmax,nfeasible,nind)
        if upd
            #println("eps:",oldeps,"->",eps)
            fopt[1]=1.0E38
        end
    end #end while
end #end timing
    ...
    if graph plotf(plotdata) end
    if verb
        println("JADE on exit μCR,μF,eps=",μCR," ",μF," ",eps)
